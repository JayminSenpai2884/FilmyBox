...
const MoviesDB = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Array containing the details of specified movies
      const specifiedMovies = [
        ...
        "Hush",
        "City Lights",
        "Silent Sonata (Ljubljana)",
        "The Red Turtle",
        "The Tribe (Plemya)"
      ];

      // Filter fetched movies based on specified movie titles
      const filteredMovies = result.filter((movie: Movie) =>
        specifiedMovies.includes(movie.title)
      );
      
      setMovies(filteredMovies);
      await getVid(filteredMovies);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-10">
      <h1 className="text-3xl font-semibold mb-8 text-white">Accessibility Movies</h1>
